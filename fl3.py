...
class LearningSwitch (object):
    def __init__ (self, connection, transparent):
        ...
        self.firewall_tcp = dict()  # add rules
        self.firewall_udp = dict()  # add rules
        self.firewall_icmp = dict()  # add rules
    ...
    def add_rule(self):
        for src,dst in self.fw_tcp:
            self.firewall_tcp[(src,dst)] = True
        for src,dst in self.fw_udp:
            self.firewall_udp[(src,dst)] = True
        for src,dst in self.fw_icmp:
            self.firewall_icmp[(src,dst)] = True
    ...
    def _handle_PacketIn (self, event):
        ...
        if isinstance(packet.next,ipv4):
            if packet.find('tcp') is not None:  # tcp packets dropped
                if self.firewall_tcp.get((packet.next.srcip,packet.next.dstip),False):
                    log.debug("blocked tcp packet from %s to %s",packet.next.srcip,packet.next.dstip)
                    drop()
                    return
            if packet.find('udp') is not None:  # tcp packets dropped
                if self.firewall_udp.get((packet.next.srcip,packet.next.dstip),False):
                    log.debug("blocked udp packet from %s to %s",packet.next.srcip,packet.next.dstip)
                    drop()
                    return
            if packet.find('icmp') is not None:  # tcp packets dropped
                if self.firewall_icmp.get((packet.next.srcip,packet.next.dstip),False):
